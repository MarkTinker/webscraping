...
class base_yarns extends Base {
	...
	public function select(){
		...
		$sql="SELECT idyarns,url,name,max1,max2,max3,max4,max5,max6 FROM yarns WHERE idyarns = '".mysql_real_escape_string($this->idyarns)."'";
	...
	public function insert(){
		$consulta=new Consulta();
		$sql = "INSERT INTO yarns ( url,name,max1,max2,max3,max4,max5,max6 ) VALUES ( '".mysql_real_escape_string($this->url)."','".mysql_real_escape_string($this->name)."','".mysql_real_escape_string($this->max1)."','".mysql_real_escape_string($this->max2)."','".mysql_real_escape_string($this->max3)."','".mysql_real_escape_string($this->max4)."','".mysql_real_escape_string($this->max5)."','".mysql_real_escape_string($this->max6)."' )";
		$consulta->Execute($sql);
	...
	public static function selectarray($where=""){
		...
		$arreglo=array();
		$objeto=new yarns();
		$sql="SELECT A.idyarns,A.url,A.name,A.max1,A.max2,A.max3,A.max4,A.max5,A.max6 FROM yarns A";
		$sql.=" $where";
		$registros= $consulta->Execute($sql);
		while ($fila=@mysql_fetch_array($registros)){
			list($objeto->idyarns,$objeto->url,$objeto->name,$objeto->max1,$objeto->max2,$objeto->max3,$objeto->max4,$objeto->max5,$objeto->max6)=$fila;
			$objeto->_select=1;
			$arreglo[]=$objeto;
			$objeto=new yarns();
		}
